...
export function DomainLinkedOncogeriatricCheckForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.message ?? "Não foi possível salvar o checkpoint.");
      setMessage("Check salvo. Quando vinculado a uma consulta, os domínios daquela avaliação passam a integrar a trajetória oncogeriátrica.");
      router.refresh();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Não foi possível salvar o checkpoint.");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <form className="stack" onSubmit={submit}>
      ...
      <label>Consulta existente para persistência por domínio
      ...
      <p className="muted">O vínculo é explícito: a Oncogeriatria reutiliza os domínios e escalas já registrados nessa consulta, sem copiar ou recalcular resultados.</p>
      ...
      <button disabled={pending} type="submit">{pending ? "Salvando…" : "Registrar Oncogeriatric Check"}</button>
