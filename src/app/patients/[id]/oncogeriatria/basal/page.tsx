  ...
  return (
    <main className="shell">
      ...
      <section className="two-columns">
        ...
        <article className="panel"><h2>Histórico basal</h2>{baselines.length ? <ul className="clean-list">{baselines.map((item) => <li key={item.id}><strong>{formatClinicalDate(item.occurredAt)}</strong><span>{item.status} · {item.consultationId ? "vinculado à consulta e aos domínios registrados nela" : "sem consulta vinculada"}</span></li>)}</ul> : <p className="muted">Ainda não há baseline registrado.</p>}</article>
      </section>
